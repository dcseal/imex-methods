...
void ConstructL( const dTensor2* node, dTensorBC2* q, 
    dTensorBC2* aux, dTensorBC2* L )
{
    ...
    for( int i=1; i <= mx; i++ )
    {
        ...
        {
            // flux term
            const double fl = -( F->get(i+1,m) - F->get(i,m) ) / dx;
    ...
    dTensorBC2* aux, dTensorBC2* L )
{
    ...
    switch( mbc )
    {
        ...
        case 1:
            ...
            // centered stencil 
            stencil->set(0, -1.0/dx);
            stencil->set(1,  1.0/dx);
            break;

        case 2:

            // centered stencil
            stencil->set( -1,  1.0  / (12.0*dx) );
            stencil->set(  0, -15.0 / (12.0*dx) );
            stencil->set(  1,  15.0 / (12.0*dx) );
            stencil->set(  2, -1.0  / (12.0*dx) );
            break;
    }
    ...
    { 
        ...
        // evaluate the flux function at the interface
        dTensor1 fval( meqn );
        flux_func_imp( node->get(i,1), qimh, fval );
